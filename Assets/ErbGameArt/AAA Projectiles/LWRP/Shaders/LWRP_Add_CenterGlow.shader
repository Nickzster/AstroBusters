Shader "ErbGameArt/LWRP/Particles/Add_CenterGlow"
{
	Properties
	{
		...
		_TilingMainTexUVNoiseZW("Tiling MainTex UV / Noise ZW", Vector) = (1,1,1,1)
	...
	SubShader
	{
		...
		Pass
		{
		    ...
		    half4 frag( GraphVertexOutput IN  ) : SV_Target
		    {
				...
				float2 appendResult57 = (float2(_TilingMainTexUVNoiseZW.x , _TilingMainTexUVNoiseZW.y));
				...
				float4 tex2DNode13 = tex2D( _MainTex, ( appendResult57 * ( ( appendResult21 * _Time.y ) + temp_output_54_0 ) ) );
				float2 appendResult59 = (float2(_TilingMainTexUVNoiseZW.z , _TilingMainTexUVNoiseZW.w));
				float2 appendResult22 = (float2(_SpeedMainTexUVNoiseZW.z , _SpeedMainTexUVNoiseZW.w));
				float4 tex2DNode14 = tex2D( _Noise, ( appendResult59 * ( temp_output_54_0 + ( _Time.y * appendResult22 ) ) ) );
