Shader "ErbGameArt/Particles/Blend_CenterGlow" {
    Properties {
        ...
        _SpeedMainTexUVNoiseUV ("Speed MainTex U/V + Noise Z/W", Vector) = (0,0,0,0)
        _Opacity ("Opacity", Range(0, 1)) = 1
        [MaterialToggle] _Usedepth ("Use depth?", Float ) = 0
        _Depthpower ("Depth power", Float ) = 1
        [MaterialToggle] _Usecenterglow ("Use center glow?", Float ) = 0
        _Mask ("Mask", 2D) = "white" {}
		[HideInInspector]_TilingMainTexUVNoiseZW("Tiling MainTex UV / Noise ZW", Vector) = (1,1,1,1)
    }
    SubShader {
        Tags {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float2 ttt = (((_Time.g*float2(_SpeedMainTexUVNoiseUV.r,_SpeedMainTexUVNoiseUV.g))+i.uv0)*float2(_TilingMainTexUVNoiseZW.r,_TilingMainTexUVNoiseZW.g));
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(ttt, _MainTex));
                float2 tt = (((_Time.g*float2(_SpeedMainTexUVNoiseUV.b,_SpeedMainTexUVNoiseUV.a))+i.uv0)*float2(_TilingMainTexUVNoiseZW.b,_TilingMainTexUVNoiseZW.a));
